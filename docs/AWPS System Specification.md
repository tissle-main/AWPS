...
## 7. Алгоритми роботи системи

### 7.1 Алгоритм роботи IoT-пристрою

Після запуску IoT-пристрій виконує ініціалізацію апаратних та програмних компонентів і визначає поточний режим роботи.

У режимі роботи IoT-пристрій виконує такі дії:

1. Перевіряє коректність часу RTC.
2. Якщо час RTC некоректний, підключається до Wi-Fi та синхронізує час із сервером часу.
3. Зчитує дані з усіх підключених датчиків та формує вимірювання.
4. Зберігає отримане вимірювання у локальному сховищі.
5. Якщо процес автоматичного поливу не виконується:
   1. Підключається до Wi-Fi.
   2. Встановлює з'єднання із сервером.
   3. Передає серверу накопичені вимірювання та повідомлення.
   4. Отримує від сервера нові або змінені налаштування.
6. На основі отриманого вимірювання розраховує `DrynessIndex`.
7. За результатом роботи алгоритму автоматичного поливу, за потреби, активує насос та виконує процес поливу.
8. Переходить у режим сну до наступного запланованого вимірювання.

У режимі налаштування IoT-пристрій виконує такі дії:

1. Активує власну Wi-Fi точку доступу.
2. Очікує підключення клієнтської частини системи.
3. Активує локальний HTTP-сервер та обробляє запити клієнтської частини.
4. Після завершення налаштування виконує перезапуск.

### 7.2 Алгоритм автоматичного поливу

Алгоритм автоматичного поливу виконується на IoT-пристрої після отримання нового вимірювання.

1. Якщо процес поливу вже активний:
   1. Перевірити кількість виконаних циклів поливу.
   2. Якщо кількість циклів досягла `MaxWateringCycles`, завершити процес автоматичного поливу.
   3. Якщо максимальну кількість циклів не досягнуто, очікувати завершення `WateringCycleInterval` та виконати наступне вимірювання.
2. Якщо процес поливу не активний:
   1. Отримати останні актуальні значення вимірювань.
   2. Розрахувати `DrynessIndex` за заданою функцією сухості.
   3. Порівняти отримане значення `DrynessIndex` із `StartWateringThreshold`.
   4. Якщо значення `DrynessIndex` досягло або перевищило `StartWateringThreshold`, активувати процес автоматичного поливу.
   5. Активувати насос на тривалість, задану параметром `WateringDuration`.
   6. Після завершення циклу поливу збільшити лічильник виконаних циклів.

### 7.3 Алгоритм синхронізації даних

Синхронізація даних виконується IoT-пристроєм за наявності з'єднання із сервером.

1. Встановити з'єднання із сервером.
2. Передати серверу локально збережені вимірювання та повідомлення.
3. Дочекатися підтвердження успішного збереження кожного переданого повідомлення.
4. Після отримання підтвердження видалити відповідні дані з локального сховища.
5. Якщо передача або збереження завершилися помилкою чи підтвердження не отримано, зберегти відповідні дані локально.
6. Непередані дані передати під час наступного циклу синхронізації.

Окремий механізм повторної відправки одного повідомлення не передбачається.

### 7.4 Алгоритм застосування налаштувань

Зміна налаштувань виконується через сервер та передається на відповідний IoT-пристрій.

1. Користувач змінює налаштування через клієнтську частину.
2. Клієнтська частина передає нові налаштування серверу.
3. Сервер зберігає нові налаштування.
4. Сервер передає нові або змінені налаштування відповідному IoT-пристрою.
5. IoT-пристрій отримує та застосовує налаштування.
6. IoT-пристрій передає серверу підтвердження успішного застосування налаштувань.

### 7.5 Алгоритм обробки подій та помилок

Події та помилки можуть виникати на IoT-пристрої або сервері.

Для подій та помилок, що виникли на IoT-пристрої:

1. Сформувати об'єкт `Event`.
2. Зберегти подію локально.
3. Передати подію серверу під час синхронізації.
4. Сервер зберігає отриману подію.
5. Сервер передає IoT-пристрою підтвердження успішного збереження.
6. Після отримання підтвердження IoT-пристрій видаляє подію з локального сховища.
7. Сервер передає інформацію про подію клієнтській частині.
8. Клієнтська частина формує та передає користувачу push-повідомлення.

Для подій та помилок, що виникли на сервері:

1. Сервер формує об'єкт `Event`.
2. Зберігає подію.
3. Передає інформацію про подію клієнтській частині.
4. Клієнтська частина формує та передає користувачу push-повідомлення.

## 8. Інтерфейс користувача

### 8.1 Загальні положення

Клієнтська частина системи повинна забезпечувати користувачу можливість перегляду стану рослин, отримання інформації про роботу системи та керування профілями рослин і налаштуваннями IoT-пристроїв.

Інтерфейс повинен складатися з основних розділів для роботи з профілями рослин, налаштуваннями пристроїв та початковою конфігурацією IoT-пристроїв.

### 8.2 Список профілів рослин

Клієнтська частина повинна містити сторінку зі списком створених профілів рослин.

Для кожного профілю повинні відображатися щонайменше:
- назва рослини;
- стан пов'язаного IoT-пристрою;
- час останнього отриманого вимірювання;
- час останньої успішної синхронізації;
- наявність нових повідомлень або помилок.

Користувач повинен мати можливість:
- відкрити профіль рослини;
...
Сторінка профілю рослини повинна містити інформацію про відповідну рослину та пов'язаний IoT-пристрій.

На сторінці повинні відображатися:
- назва рослини;
- ідентифікатор пов'язаного IoT-пристрою;
- час останнього оновлення даних;
- графік вимірювань;
- повідомлення та помилки, пов'язані з профілем.

Графік повинен забезпечувати:
- відображення зміни вимірюваних параметрів у часі;
- вибір параметра для відображення;
- вибір часового діапазону;
- масштабування графіка;
- перегляд точного значення вимірювання та часу його отримання.

### 8.4 Повідомлення

У профілі рослини повинні відображатися повідомлення, пов'язані з роботою IoT-пристрою та сервера.
...
### 8.5 Налаштування пристрою

Клієнтська частина повинна містити окремий розділ для налаштування параметрів роботи IoT-пристрою.

Користувач повинен мати можливість змінювати:
- інтервал вимірювань;
- поріг початку автоматичного поливу;
- максимальну кількість циклів поливу;
- тривалість одного циклу поливу;
- інтервал між циклами поливу.

Після зміни налаштувань вони повинні передаватися на сервер та застосовуватися відповідним IoT-пристроєм.

### 8.6 Початкова конфігурація IoT-пристрою

Для початкового налаштування IoT-пристрою клієнтська частина повинна містити окрему сторінку конфігурації пристрою.

Сторінка повинна забезпечувати:
- підключення клієнта до Wi-Fi мережі IoT-пристрою;
- введення облікових даних Wi-Fi мережі, до якої повинен підключитися IoT-пристрій;
- введення або передачу ідентифікатора сервера;
- передачу параметрів конфігурації на IoT-пристрій;
- відображення результату виконання операції конфігурації.

Після завершення початкової конфігурації IoT-пристрій повинен перезапуститися та перейти до штатного режиму роботи.

## 9. Вимоги до програмної реалізації

### 9.1 Загальні вимоги

Програмна реалізація системи повинна базуватися на платформі **.NET**.

Компоненти системи повинні розроблятися з використанням єдиного технологічного стеку, наскільки це дозволяють особливості відповідної платформи.

### 9.2 IoT-пристрій

Програмне забезпечення IoT-пристрою повинно розроблятися з використанням **.NET nanoFramework**.

Програмне забезпечення IoT-пристрою повинно забезпечувати:
- роботу з підключеними датчиками;
- локальне зберігання вимірювань та подій;
- керування насосом;
- виконання алгоритму автоматичного поливу;
- роботу з RTC;
- підключення до Wi-Fi;
- MQTT-взаємодію із сервером;
- HTTP-взаємодію з клієнтською частиною під час початкової конфігурації;
- перехід у режим низького енергоспоживання між вимірюваннями.

### 9.3 Серверна частина

Серверна частина повинна розроблятися на платформі **.NET**.

Сервер повинен забезпечувати:
- HTTP API для взаємодії з клієнтською частиною;
- MQTT-взаємодію з IoT-пристроями;
- зберігання профілів рослин;
- зберігання налаштувань;
- зберігання історії вимірювань;
- зберігання подій;
- ідентифікацію та авторизацію IoT-пристроїв;
- передачу налаштувань на IoT-пристрої;
- обробку підтверджень від IoT-пристроїв.

### 9.4 Клієнтська частина

Клієнтська частина повинна реалізовуватися з використанням **Blazor Hybrid**.

Клієнтська частина повинна забезпечувати роботу на визначених цільових платформах та надавати користувачу доступ до функцій, описаних у розділі 8.

### 9.5 Взаємодія компонентів

Для обміну даними між компонентами системи повинні використовуватися:

- **MQTT** — для взаємодії IoT-пристрою із сервером;
- **HTTP** — для взаємодії клієнтської та серверної частин;
- **HTTP** — для початкової взаємодії клієнтської частини з IoT-пристроєм.

MQTT-повідомлення повинні передаватися у бінарному форматі відповідно до структур, визначених у розділі 5.

### 9.6 Вимоги до програмної структури

Програмна реалізація повинна забезпечувати модульність компонентів системи.

Алгоритм автоматичного поливу повинен бути відокремлений від інших функцій IoT-пристрою, щоб забезпечити можливість його зміни або розширення без зміни основної логіки роботи пристрою.

Реалізація роботи з датчиками повинна дозволяти заміну датчика того самого типу без зміни функціональності системи.

### 9.7 Обробка помилок та логування

Програмне забезпечення повинно забезпечувати обробку помилок, що виникають під час:
- роботи датчиків;
...
Помилки та інші значущі події повинні реєструватися відповідно до структури `Event`, визначеної у розділі 6.